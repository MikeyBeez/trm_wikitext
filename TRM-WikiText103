"""
TRM-WikiText103: Chunked Autoregressive Transformer with Recursive Refinement
Character-level TRM implementation preserving all critical mechanisms.

Filename: trm_wk3.py
Author: Michael Bonsignore

KEY MECHANISMS:
1. WARM START: Initialize draft from target tokens
2. DETACH: Between refinement rounds (except final)
3. NO MASK: Bidirectional attention within chunk enables token negotiation
4. DEEP SUPERVISION: Loss calculated at every refinement step

WARNING: This is CHARACTER-LEVEL tokenization. For word-level experiments 
(like the 40.41 ppl WikiText-103 result), use bigger.py with tiktoken.
"""
...
def main():
    ...
    print("Expected results: ~150-200 perplexity (not 40.41)")
    print()
    ...
    try:
        ...
        print("For quick testing, download tiny_shakespeare.txt manually and modify script.")
        raise
